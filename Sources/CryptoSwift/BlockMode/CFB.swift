...
public struct CFB: BlockMode {
  ...
  public func worker(blockSize: Int, cipherOperation: @escaping CipherOperationOnBlock, encryptionOperation: @escaping CipherOperationOnBlock) throws -> CipherModeWorker {
    if !(self.iv.count == blockSize || (self.segmentSize == .cfb8 && self.iv.count == AES.blockSize)) {
      throw Error.invalidInitializationVector
    }

    return CFBModeWorker(blockSize: blockSize, iv: self.iv.slice, segmentSize: self.segmentSize, cipherOperation: cipherOperation)
  }
}
  ...
  mutating func encrypt(block plaintext: ArraySlice<UInt8>) -> Array<UInt8> {
    switch self.segmentSize {
      ...
      case .cfb8:
        ...
        self.prev = Array((self.prev ?? self.iv).dropFirst()) + [result[0]]
        return result
    }
  }
  ...
  mutating func decrypt(block ciphertext: ArraySlice<UInt8>) -> Array<UInt8> {
    switch self.segmentSize {
      ...
      case .cfb8:
        ...
        self.prev = Array((self.prev ?? self.iv).dropFirst()) + [Array(ciphertext)[0]]
