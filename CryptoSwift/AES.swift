...
final public class AES {
    ...
    public lazy var expandedKey2:[[UInt32]] = { self.expandKey2(self.key, variant: self.variant) }()
    ...
    private let T1:[UInt32] =  [0xa56363c6, 0x847c7cf8, 0x997777ee, 0x8d7b7bf6,
    ...
    private let T2:[UInt32] =  [0x6363c6a5, 0x7c7cf884, 0x7777ee99, 0x7b7bf68d,
    ...
    private let T3:[UInt32] =  [0x63c6a563, 0x7cf8847c, 0x77ee9977, 0x7bf68d7b,
    ...
    private let T4:[UInt32] =  [0xc6a56363, 0xf8847c7c, 0xee997777, 0xf68d7b7b,
    ...
    */
    ...
    private func B0(x: UInt32) -> UInt8 {
        return UInt8(x & 0xFF)
    }
    
    private func B1(x: UInt32) -> UInt8 {
        return UInt8((x >> 8) & 0xFF)
    }
    
    private func B2(x: UInt32) -> UInt8 {
        return UInt8((x >> 16) & 0xFF)
    }
    
    private func B3(x: UInt32) -> UInt8 {
        return UInt8((x >> 24) & 0xFF)
    }
    
    private func F1(x0: UInt32, _ x1: UInt32, _ x2: UInt32, _ x3: UInt32) -> UInt32 {
        var result:UInt32 = 0
        result |= UInt32(B1(T1[Int(x0 & 255)]))
        result |= UInt32(B1(T1[Int((x1 >> 8) & 255)])) << 8
        result |= UInt32(B1(T1[Int((x2 >> 16) & 255)])) << 16
        result |= UInt32(B1(T1[Int(x3 >> 24)])) << 24
        return result
    }
    ...
    private func encryptBlock(block:[UInt8]) -> [UInt8]? {
        ...
        let rk = self.expandedKey2
        
        // replace expanedKey [UInt8] into expanedKey[[UInt32]]
        ...
        for r in 0..<rounds - 1 {
            ...
            b[0] = T1[Int(t[0]) & 0xFF] ^ T2[Int(t[1] >> 8) & 0xFF] ^ T3[Int(t[2] >> 16) & 0xFF] ^ T4[Int(t[3] >> 24)]
            b[1] = T1[Int(t[1]) & 0xFF] ^ T2[Int(t[2] >> 8) & 0xFF] ^ T3[Int(t[3] >> 16) & 0xFF] ^ T4[Int(t[0] >> 24)]
            b[2] = T1[Int(t[2]) & 0xFF] ^ T2[Int(t[3] >> 8) & 0xFF] ^ T3[Int(t[0] >> 16) & 0xFF] ^ T4[Int(t[1] >> 24)]
            b[3] = T1[Int(t[3]) & 0xFF] ^ T2[Int(t[0] >> 8) & 0xFF] ^ T3[Int(t[1] >> 16) & 0xFF] ^ T4[Int(t[2] >> 24)]            
        }
        ...
        for e in b.lazy.enumerate() {
            let num = b[Int(e.index)]
    ...
    private func expandKey2(key:[UInt8], variant:AESVariant) -> [[UInt32]] {
