...
final class ChaCha20Tests: XCTestCase {
  ...
  func testChaCha20ExplicitCounterMatchesBlockCounter() {
    let key = Array<UInt8>(0..<32)
    let counter: Array<UInt8> = [1, 0, 0, 0]
    let iv = Array<UInt8>(repeating: 0, count: 12)
    let message = Array<UInt8>(repeating: 0, count: 64)

    do {
      let explicitCounter = try ChaCha20(key: key, counter: counter, iv: iv).encrypt(message)
      let blockCounter = try ChaCha20(key: key, iv: iv, blockCounter: 1).encrypt(message)
      XCTAssertEqual(explicitCounter, blockCounter)
    } catch {
      XCTFail("\(error)")
    }
  }
...
extension ChaCha20Tests {
  static func allTests() -> [(String, (ChaCha20Tests) -> () -> Void)] {
    let tests = [
      ...
      ("testChaCha20ExplicitCounterMatchesBlockCounter", testChaCha20ExplicitCounterMatchesBlockCounter)
    ]
